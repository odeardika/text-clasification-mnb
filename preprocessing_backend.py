...
def do_preprocessing():
    ...
    reviews = review_datas['Reviews'][:200]
    ...
    system.list_to_csv(reviews_stem,'Result_Preprocessing/Stem_result.csv')
    print(10)
    ...
    tfidf_model = model_prediction_accuracy(X=temp,y=label_reviews,model_name='tfidf')
    temp = pd.read_csv('Result_Preprocessing/One Hot Encoder_result.csv').iloc[:,1:]
    one_hot_model = model_prediction_accuracy(X=temp,y=label_reviews,model_name='one_hot')
    temp = pd.read_csv('Result_Preprocessing/BOW_result.csv').iloc[:,1:]
    bow_model = model_prediction_accuracy(X=temp,y=label_reviews,model_name='bow')
